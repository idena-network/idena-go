...
type Database struct {
	diskdb idenadb.Database // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb idenadb.Database) *Database {
	return &Database{
		diskdb:    diskdb,
...
func (db *Database) Cap(limit common.StorageSize) error {
	...
	if flushPreimages {
		for hash, preimage := range db.preimages {
			...
			if batch.ValueSize() > idenadb.IdealBatchSize {
...
func (db *Database) commit(hash common.Hash, batch idenadb.Batch) error {
