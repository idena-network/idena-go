...
// test all variations.
func makeProvers(trie *Trie) []func(key []byte) *idenadb.MemDatabase {
	var provers []func(key []byte) *idenadb.MemDatabase

	// Create a direct trie based Merkle prover
	provers = append(provers, func(key []byte) *idenadb.MemDatabase {
		proof := idenadb.NewMemDatabase()
...
func BenchmarkVerifyProof(b *testing.B) {
	...
	root := trie.Hash()
	var keys []string
	var proofs []*idenadb.MemDatabase
	for k := range vals {
		keys = append(keys, k)
		proof := idenadb.NewMemDatabase()
