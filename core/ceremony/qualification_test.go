...
func Test_qualifyOneFlip(t *testing.T) {
	...
	ans := fillArray(6, 1, 1)
	q := qual.qualifyOneFlip(ans, 4, 18, 4, 8, 0)
	require.Equal(Qualified, q.status)
	require.Equal(types.Left, q.answer)
	require.Equal(types.GradeA, q.grade)

	ans = fillArray(6, 0, 0)
	q = qual.qualifyOneFlip(ans, 2, 6, 2, 4, 0)
	require.Equal(Qualified, q.status)
	require.Equal(types.Left, q.answer)
	require.Equal(types.GradeC, q.grade)

	ans = fillArray(7, 0, 0)
	q = qual.qualifyOneFlip(ans, 2, 8, 2, 4, 0)
	require.Equal(Qualified, q.status)
	require.Equal(types.Left, q.answer)
	require.Equal(types.GradeD, q.grade)

	ans = fillArray(7, 0, 0)
	q = qual.qualifyOneFlip(ans, 3, 12, 3, 6, 0)
	require.Equal(Qualified, q.status)
	require.Equal(types.Left, q.answer)
	require.Equal(types.GradeB, q.grade)

	ans = fillArray(6, 1, 1)
	q = qual.qualifyOneFlip(ans, 3, 13, 3, 6, 0)
	require.Equal(Qualified, q.status)
	require.Equal(types.Left, q.answer)
	require.Equal(types.GradeB, q.grade)

	ans = fillArray(6, 1, 1)
	q = qual.qualifyOneFlip(ans, 4, 12, 4, 8, 0)
	require.Equal(Qualified, q.status)
	require.Equal(types.Left, q.answer)
	require.Equal(types.GradeC, q.grade)

	ans = fillArray(6, 1, 1)
	q = qual.qualifyOneFlip(ans, 4, 8, 4, 8, 0)
	require.Equal(Qualified, q.status)
	require.Equal(types.Left, q.answer)
	require.Equal(types.GradeD, q.grade)

	ans = fillArray(6, 1, 1)
	q = qual.qualifyOneFlip(ans, 5, 6, 3, 8, 0)
	require.Equal(Qualified, q.status)
	require.Equal(types.Left, q.answer)
	...
	q = qual.qualifyOneFlip(ans, 3, 0, 0, 3, 0)
	require.Equal(types.GradeReported, q.grade)

	ans = fillArray(1, 2, 00)
	q = qual.qualifyOneFlip(ans, 2, 0, 0, 2, 0)
	require.Equal(types.GradeReported, q.grade)

	ans = fillArray(1, 2, 00)
	q = qual.qualifyOneFlip(ans, 2, 0, 1, 3, 0)
	...
	q = qual.qualifyOneFlip(ans, 3, 0, 2, 5, 0)
	require.NotEqual(types.GradeReported, q.grade)

	ans = fillArray(3, 2, 00)
	q = qual.qualifyOneFlip(ans, 4, 0, 0, 4, 0)
	require.Equal(types.GradeReported, q.grade)

	ans = fillArray(3, 2, 00)
	q = qual.qualifyOneFlip(ans, 4, 0, 1, 5, 0)
	require.Equal(types.GradeReported, q.grade)
	...
	ans = fillArray(3, 3, 00)
	q = qual.qualifyOneFlip(ans, 4, 0, 2, 6, 0)
	require.Equal(types.GradeReported, q.grade)

	ans = fillArray(2, 3, 0)
	q = qual.qualifyOneFlip(ans, 3, 10, 2, 5, 0)
	require.Equal(types.GradeD, q.grade)
}
