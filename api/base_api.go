...
func (api *BaseApi) getTx(from common.Address, to *common.Address, txType types.TxType, amount decimal.Decimal,
	...
	if txType == types.KillTx {
		amount = blockchain.ConvertToFloat(state.State.GetStakeBalance(from))
	}
	if txType == types.KillInviteeTx && to != nil {
		amount = blockchain.ConvertToFloat(state.State.GetStakeBalance(*to))
	}
